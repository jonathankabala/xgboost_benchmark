...
def get_dmatrix_cpu(py_dir_name, config):
    ...
    del X, y, train_idx, test_idx; gc.collect()
...
def get_py_data(config, args):
    ...
    py_dir_name = Path(args.data_dir) / f"py_data" / f"samples_{config.sample.n_samples}"
...
def py_one_run(
    ...
    params["nthread"] = args.threads # still limit the number of gpu even if we are using gpu since some other operations are on cpu
